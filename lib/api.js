...
export const guestApi = {
  ...
  verifyPhone: (token, phone) =>
    request(`/api/v1/room/${token}/verify`, {
      method: 'POST',
      body: JSON.stringify({ phone }),
    }),

  getMessages: (token) => request(`/api/v1/room/${token}/chat`),
  sendMessage: (token, body) =>
    request(`/api/v1/room/${token}/chat`, {
      method: 'POST',
...
export const staffApi = {
  ...
  checkIn: (token, data) =>
    request('/api/v1/staff/checkin', {
      method: 'POST',
      headers: staffHeaders(token),
      body: JSON.stringify(data),
    }),

  checkOut: (token, reservationId) =>
    request(`/api/v1/staff/checkout/${reservationId}`, {
      method: 'POST',
      headers: staffHeaders(token),
    }),

  getChatList: (token) =>
    request('/api/v1/staff/chat', { headers: staffHeaders(token) }),

  getChatMessages: (token, roomId) =>
    request(`/api/v1/staff/chat/${roomId}`, { headers: staffHeaders(token) }),

  sendChatMessage: (token, roomId, body) =>
    request(`/api/v1/staff/chat/${roomId}`, {
      method: 'POST',
      headers: staffHeaders(token),
      body: JSON.stringify({ body }),
    }),
