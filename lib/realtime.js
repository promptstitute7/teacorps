...
/**
 * Subscribe to ticket updates and new tickets for a specific room (guest view).
 * @param {string} roomId  — Prisma room.id (UUID)
 * @param {{ onUpdate, onInsert }} handlers
 * @returns channel
 */
export function subscribeToGuestTickets(roomId, { onUpdate, onInsert } = {}) {
  const client = getClient();
