...
const OPTIONS = [
  { id: 'clean_room',         label: 'Clean My Room',      icon: 'cleaning_services', desc: 'Full room cleaning' },
  { id: 'fresh_towels',       label: 'Fresh Towels',       icon: 'dry',               desc: 'Replace bathroom towels' },
  { id: 'change_linen',       label: 'Change Bed Linen',   icon: 'bed',               desc: 'Fresh sheets & pillowcases' },
  { id: 'restock_toiletries', label: 'Restock Toiletries', icon: 'soap',              desc: 'Shampoo, soap & essentials' },
  { id: 'dental_kit',         label: 'Dental Kit',         icon: 'dentistry',         desc: 'Toothbrush & toothpaste' },
  { id: 'slippers',           label: 'Slippers',           icon: 'footprint',         desc: 'Comfortable room slippers' },
  { id: 'dnd',                label: 'Do Not Disturb',     icon: 'do_not_disturb_on', desc: 'Pause all housekeeping' },
];
...
export default function HousekeepingPage() {
  ...
  async function handleSubmit() {
    ...
    if (selected === 'dnd') {
      ...
      setTicket({ ticketNumber: 'DND' });
      setSubmitting(false);
      return;
    }
    try {
      const opt = OPTIONS.find(o => o.id === selected);
      const created = await guestApi.createTicket(token, { category: 'housekeeping', subcategory: opt?.label, description: note || opt?.label });
  ...
  if (ticket) return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center px-6 text-center">
      ...
      <h2 className="text-2xl font-semibold text-on-surface mb-2">{ticket.ticketNumber === 'DND' ? 'Do Not Disturb On' : 'Request Sent'}</h2>
      {ticket.ticketNumber !== 'DND' && <p className="text-sm text-on-surface-variant mb-1">Ticket #{ticket.ticketNumber}</p>}
      <p className="text-xs text-on-surface-variant mb-10">{ticket.ticketNumber === 'DND' ? 'Your privacy will be respected.' : 'Housekeeping has been notified.'}</p>
    ...
    <div className="min-h-screen bg-background pb-32">
      <header className="fixed top-0 left-0 right-0 z-50 bg-background/95 glass-nav border-b border-outline-variant/30">
        <div className="flex items-center gap-4 px-5 py-4 max-w-lg mx-auto">
          ...
          <div>
            ...
            <h1 className="text-lg font-semibold text-primary leading-tight">Housekeeping</h1>
          </div>
        </div>
      </header>

      <main className="pt-24 px-5 max-w-lg mx-auto">
        <p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-on-surface-variant mb-4">Select a service</p>
        ...
        <div className="bg-white rounded-xl border border-outline-variant/40 px-4 py-3 mb-6 shadow-sm">
          <p className="text-[10px] uppercase tracking-[0.1em] text-on-surface-variant font-semibold mb-2">Additional notes</p>
          <textarea placeholder="Any special instructions..." value={note} onChange={e => setNote(e.target.value)} rows={2}
      ...
      <div className="fixed bottom-[72px] left-0 right-0 px-5">
        <button onClick={handleSubmit} disabled={!selected || submitting}
          ...
          {submitting ? <Spinner size="sm" /> : (<><span className="material-symbols-outlined text-[16px]">send</span>Submit Request</>)}
        </button>
      </div>
